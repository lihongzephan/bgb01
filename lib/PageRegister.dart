...
//    Navigator.pushNamedAndRemoveUntil(context, '/', (_) => false);
  ...
  void funRegisterPressed() async {
    ...
    if (!gv.bolLoading) {
      ...
      if (!ut.isAllowedChars(ctlUserID.text)) {
        setState(() {
          gv.strRegisterError = ls.gs('UserIDErrorAllowedChars');
        });
        return;
      }
      ...
      if (ctlUserPW.text != ctlUserPWConfirm.text) {
        setState(() {
        ...
        while (gv.bolLoading) {
          ...
          if (gv.aryRegisterResult.toString() == '[]') {
            ...
            if (gv.aryRegisterResult[0] == '0000') {
              ...
              gv.gstrCurPage = 'Login';
              gv.strLoginError = ls.gs('RegisterSuccess');
              Navigator.pushAndRemoveUntil(context,
                MaterialPageRoute(builder: (BuildContext context) => ClsLogin()),
                    (_) => false,
